...
exports.updatePortfolio = async (req, res) => {
  ...
  try {
    ...
    const oldVideoPath = portfolioRows[0].video ? path.join(__dirname, "..", portfolioRows[0].video) : null;
    ...
    const oldImagePaths = oldImageRows.map(row => path.join(__dirname, "..", row.image));
    ...
    await connection.query("DELETE FROM portfolio_keywords WHERE portfolio_id = ?", [portfolio_id]);
    ...
    await connection.query("DELETE FROM portfolio_images WHERE portfolio_id = ?", [portfolio_id]);
